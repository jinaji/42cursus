...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/23 18:36:33 by jinkim2           #+#    #+#             */
/*   Updated: 2022/07/23 18:37:57 by jinkim2          ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
void	image_to_window(t_sl *s, int w, int h)
{
	...
	else if ((s->m[h][w] == 'P') && (s->loc.move % 2))
		mlx_put_image_to_window(s->mlx, \
		s->win, s->img.cha1, 64 * w, 64 * h);
	else if ((s->m[h][w] == 'P') && !(s->loc.move % 2))
		mlx_put_image_to_window(s->mlx, \
		s->win, s->img.cha2, 64 * w, 64 * h);
	else if (s->m[h][w] == 'E')
		mlx_put_image_to_window(s->mlx, \
...
void	init_image(t_sl *s)
{
	...
	s->img.cha2 = \
	mlx_xpm_file_to_image(s->mlx, "./image/character2.xpm", &width, &height);
	s->img.col = \
